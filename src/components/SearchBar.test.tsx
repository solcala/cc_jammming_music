...
it('renders search section', () => {
  render(
    <SearchBar search={() => {}} searchBy="Song" setSearchBy={() => {}} />,
  );
...
it('shows inline error when searching with an empty query', async () => {
  ...
  render(<SearchBar search={() => {}} searchBy="" setSearchBy={() => {}} />);
...
it('associates the search input with a visible label', () => {
  ...
  expect(screen.getByLabelText('Search by song title')).toBe(
    screen.getByTestId('search-by-input'),
  );
});
